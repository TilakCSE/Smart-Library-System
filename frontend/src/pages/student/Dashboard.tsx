...
import { BookOpen, Clock, AlertTriangle, ArrowRight, MapPin, LogOut } from "lucide-react";
import StudentSearchModal from "@/components/StudentSearchModal";
import { getAuth, signOut } from "firebase/auth";
import { useNavigate } from "react-router-dom";
import api from "@/lib/axios"; // Make sure this is imported at the top!
...
export default function StudentDashboard() {
  ...
  // Fetch real name from NeonDB using Firebase email
  useEffect(() => {
  ...
  // The Warning Dialog before launching the map
  const handleMapLaunch = () => {
    const isAtEntrance = window.confirm(
      "📍 Navigation Calibration\n\nFor accurate 3D routing, please ensure you are standing near the Library Entry Gates before proceeding.\n\nAre you ready?"
    );
      ...
      >
        ...
        <motion.div variants={item} className="flex justify-between items-center mb-6 text-white">
          ...
          <div className="flex items-center gap-3">
            {/* Keeping your 🎓 button for future Dev Mode triggers */}
        ...
        <motion.div variants={item} className="mb-8">
          <button 
            ...
            className="w-full group relative overflow-hidden rounded-2xl bg-slate-900 p-1"
          >
