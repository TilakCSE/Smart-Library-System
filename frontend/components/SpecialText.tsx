...
import { useInView } from "framer-motion"; // TWEAKED: Fixed import path
...
export function SpecialText({ children, speed = 20, delay = 0, className = "", inView = false, once = true }: SpecialTextProps) {
  ...
  // ... (Keep the exact same logic functions you provided here) ...
  ...
  const runPhase1 = () => {
    ...
    if (animationStep < maxSteps - 1) { setAnimationStep((prev) => prev + 1); } 
  ...
  const runPhase2 = () => {
    ...
    if (revealedCount < text.length) {
      if (animationStep % 2 === 0) { chars.push("_"); } else { chars.push(getRandomChar()); }
    }
    for (let i = chars.length; i < text.length; i++) { chars.push(getRandomChar()); }
    setDisplayText(chars.join(""));
    if (animationStep < text.length * 2 - 1) { setAnimationStep((prev) => prev + 1); } 
    else { setDisplayText(text); if (intervalRef.current) { clearInterval(intervalRef.current); intervalRef.current = null; } }
  };
  ...
  useEffect(() => {
    if (shouldAnimate && !hasStarted) {
      ...
      startTimeoutRef.current = window.setTimeout(() => { startTimeoutRef.current = null; startAnimation(); }, delay * 1000);
    }
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => { if (currentPhase === "phase1") { runPhase1(); } else { runPhase2(); } }, speed);
    return () => { if (intervalRef.current) clearInterval(intervalRef.current); };
  }, [currentPhase, animationStep, text, speed, hasStarted]);

  return (
    <span ref={containerRef} className={`h-4.5 leading-5 inline-flex font-mono font-medium ${className}`}>
      {displayText}
    </span>
  );
}