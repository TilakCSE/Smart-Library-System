...
type TransactionStatus = {
  ...
  message: string;
  fineAmount?: number;
  fineMath?: { due_date: string; return_date: string; days_late: number; }; // Add this line
};
...
export default function CirculationKiosk() {
  ...
  const handleIssue = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // FastAPI expects these as query parameters based on endpoints.py
      ...
      setIssueData({ userIdentifier: "", rfid: "", duration: 14 }); // Reset form
  ...
  const addLogEntry = (action: "Issue" | "Return", rfid: string, user?: string, logStatus: "success" | "warning" = "success") => {
    ...
    setRecentTransactions(prev => [newEntry, ...prev].slice(0, 5)); // Keep only last 5
  };

  return (
    // NEW WRAPPER: Flex layout for sidebar integration
    <div className="min-h-screen bg-stone-50 flex text-stone-900 font-sans selection:bg-stone-200">
      
      {/* 1. Sidebar Injection */}
      <AdminSidebar isServerLive={true} />

      {/* 2. Main Content offset with ml-64 */}
      <main className="flex-1 ml-64 p-6 md:p-12 h-screen overflow-y-auto">
        <div className="max-w-6xl mx-auto">
          ...
          <header className="mb-12 flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-serif tracking-tight text-stone-950 flex items-center gap-3">
                <ArrowRightLeft className="w-8 h-8 text-stone-400" /> Circulation Desk
              </h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-7">
              <div className="bg-white border border-stone-200 rounded-[2.5rem] shadow-sm p-2 overflow-hidden">
                
                {/* Custom Animated Tabs */}
                <div className="flex p-2 bg-stone-50 rounded-[2rem] relative">
                  {["issue", "return"].map((tab) => (
                    <button
                      key={tab}
                      onClick={() => {
                        setActiveTab(tab as "issue" | "return");
                        setStatus({ type: "idle", message: "" }); // Clear status on switch
                      }}
                      className={`flex-1 relative z-10 py-3.5 text-sm font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-2 ${
                    ...
                    >
                      ...
                      {activeTab === tab && (
                        <motion.div
                          ...
                          className="absolute inset-0 bg-white rounded-[1.5rem] shadow-sm -z-10 border border-stone-200/50"
                      ...
                      >
                        <div className="space-y-2">
                          ...
                          <input 
                            ...
                            className="w-full bg-stone-50 border border-stone-200 rounded-2xl px-5 py-4 text-sm font-medium text-stone-900 focus:outline-none focus:ring-2 focus:ring-stone-950/10 focus:border-stone-400 transition-all disabled:opacity-50"
                          />
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                          <div className="space-y-2 md:col-span-2">
                            ...
                            <input 
                              ...
                              className="w-full bg-stone-50 border border-stone-200 rounded-2xl px-5 py-4 text-sm font-mono text-stone-900 focus:outline-none focus:ring-2 focus:ring-stone-950/10 focus:border-stone-400 transition-all disabled:opacity-50 uppercase"
                            />
                          </div>
                          <div className="space-y-2">
                            <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-stone-500 flex items-center gap-2">
                            ...
                            <input 
                              ...
                              className="w-full bg-stone-50 border border-stone-200 rounded-2xl px-5 py-4 text-sm font-mono text-stone-900 focus:outline-none focus:ring-2 focus:ring-stone-950/10 focus:border-stone-400 transition-all disabled:opacity-50 text-center"
                            />
                          </div>
                        </div>
                        ...
                        <button 
                          type="submit" 
                          disabled={isSubmitting}
                          className="w-full mt-4 flex items-center justify-center gap-2 bg-stone-950 hover:bg-stone-800 text-white rounded-2xl py-4.5 font-bold text-xs uppercase tracking-widest transition-all shadow-lg shadow-stone-950/10 disabled:opacity-70"
                        >
            ...
            <div className="lg:col-span-5 flex flex-col gap-6">
              ...
              <div className="bg-white border border-stone-200 rounded-[2.5rem] shadow-sm p-8 min-h-[160px] flex flex-col justify-center relative overflow-hidden transition-all duration-300">
                <AnimatePresence mode="wait">
                  ...
                  <motion.div key="fine" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex flex-col items-center text-center w-full">
                    <div className="w-full bg-rose-50 border border-rose-200 rounded-2xl p-5">
                      <Wallet className="w-6 h-6 text-rose-600 mx-auto mb-3" />
                      <p className="text-xs font-bold text-rose-900 mb-1">{status.message}</p>
                      
                      {status.fineMath && (
                        <div className="mt-3 mb-3 text-left bg-rose-500/10 p-3 rounded-xl border border-rose-500/20">
                          <div className="flex justify-between text-[10px] text-rose-700 font-mono mb-1">
                            <span>Due: {status.fineMath.due_date}</span>
                            <span>Returned: {status.fineMath.return_date}</span>
                          </div>
                          <div className="flex justify-between text-[10px] text-rose-700 font-mono font-bold pt-1 border-t border-rose-500/10 mt-1">
                      ...
                      <div className="bg-white rounded-xl py-3 border border-rose-100">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-rose-400 mb-1">Total Fine Generated</p>
                        <p className="text-3xl font-black text-rose-600">₹{status.fineAmount}</p>
                      </div>
                    </div>
                  </motion.div>
                )}
              ...
              <div className="bg-white border border-stone-200 rounded-[2.5rem] shadow-sm flex-1 flex flex-col overflow-hidden">
                <div className="p-6 border-b border-stone-100 flex items-center gap-2 bg-stone-50/50">
                  <History className="w-4 h-4 text-stone-400" />
                  <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-stone-600">Session Log</h3>
                </div>
                          ...
                          >
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-[10px] font-mono text-stone-400">{tx.time}</span>
                              </div>
                              <p className="text-sm font-bold text-stone-900 font-mono uppercase">{tx.rfid}</p>
                              {tx.user && <p className="text-[10px] text-stone-500 mt-0.5">{tx.user}</p>}
                            </div>
