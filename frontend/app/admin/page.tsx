...
import { 
  ...
  Search, Activity, Loader2, Flame, Trophy
} from "lucide-react";
...
export default function AdminCommandCenter() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 p-6 md:p-12 overflow-x-hidden">
      
      {/* Top Navbar - Made Responsive */}
      <div className="flex items-center justify-end mb-8 md:mb-12">
        <div className="flex items-center gap-4">
          ...
          <div className="w-10 h-10 bg-slate-200 rounded-full border border-slate-300 flex items-center justify-center text-slate-500 font-bold shrink-0 shadow-sm">
             A
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8 md:mb-10">
        {[
          ...
          <div key={i} className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">{kpi.label}</p>
            <div className="text-2xl md:text-3xl font-black text-slate-900 flex items-baseline gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 md:mb-10">
        ...
        <div className="lg:col-span-1 bg-white border border-slate-200 p-6 md:p-8 rounded-2xl shadow-sm flex flex-col">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 p-6 md:p-8 rounded-2xl shadow-sm flex flex-col">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              <Activity className="w-5 h-5 text-indigo-600 shrink-0" />
              <h3 className="font-bold text-slate-900 text-sm md:text-base">Spatial Search Heatmap</h3>
            </div>
            <span className="hidden sm:inline-block text-xs font-semibold text-slate-500 bg-slate-100 px-3 py-1 rounded-lg shrink-0">Top Zones</span>
          </div>
          ...
          <div className="flex-1 flex flex-col justify-center">
             {heatmap.length > 0 ? (
                  ...
                  {heatmap.map((zone, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center gap-2 md:gap-4">
                        <span className="w-20 md:w-24 text-[10px] md:text-xs font-bold text-slate-600 uppercase font-mono truncate">{zone.name}</span>
                        ...
                        <span className="w-6 md:w-8 text-right text-[10px] md:text-xs font-mono text-slate-400">{zone.searches}</span>
                      </div>
                    )
                  })}
               </div>
               ...
               <div className="h-full w-full bg-slate-50 rounded-xl border border-dashed border-slate-200 flex flex-col items-center justify-center p-6">
                  ...
                  <p className="text-slate-400 text-[10px] md:text-xs font-mono tracking-widest uppercase text-center">Insufficient telemetry data.<br/>Waiting for student searches.</p>
               </div>
             )}
          </div>
        </div>
      ...
      {/* Live Transaction Ledger - Added horizontal scroll wrapper for mobile */}
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden w-full">
        <div className="px-6 py-4 md:px-8 md:py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <h3 className="font-bold text-slate-900 text-sm md:text-base">Live Master Ledger</h3>
          <button 
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap min-w-[800px]">
            <thead>
              <tr className="text-[10px] uppercase tracking-widest text-slate-400 font-bold border-b border-slate-100">
                <th className="px-6 py-4 md:px-8">Event ID</th>
                <th className="px-6 py-4 md:px-8">User</th>
                <th className="px-6 py-4 md:px-8">Action</th>
                <th className="px-6 py-4 md:px-8">Details</th>
                <th className="px-6 py-4 md:px-8">Timestamp</th>
              </tr>
            </thead>
              ...
              {ledger.map((entry, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                    <td className="px-6 py-4 md:px-8 font-mono text-xs">{entry.id.substring(0, 10)}...</td>
                    <td className="px-6 py-4 md:px-8 font-semibold text-slate-900">{entry.user_name}</td>
                    <td className="px-6 py-4 md:px-8">
                       <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${typeColor}`}>
                        {entry.action}
                      </span>
                    </td>
                    <td className="px-6 py-4 md:px-8">{entry.details}</td>
                    <td className="px-6 py-4 md:px-8 text-slate-400 text-xs">{new Date(entry.timestamp).toLocaleTimeString()}</td>
                  </tr>
                )
              })}
              {ledger.length === 0 && (
